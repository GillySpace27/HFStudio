...
IMAGE_FRAGMENTS = (
    ...
    "solarSky.frag",
    # The colour-table legend. GLSLImageShader builds it with the common prefix like the rest, so
...
# The image fragments drawn on the shared full-screen quad, so their program pairs with
# screen.vert. warpSurface.frag is the exception: it is drawn on a mesh and takes its own vertex
# stage, which is why it is listed separately below.
SCREEN_FRAGMENTS = tuple(f for f in IMAGE_FRAGMENTS if f != "warpSurface.frag")
...
def build_shader_list(temp_dir: Path) -> list[Path]:
    ...
    ignored = image_fragments | common_warp | {COMMON_FRAGMENT, COMMON_VERTEX}
    standalone = sorted(path for path in GLSL_DIR.iterdir() if path.suffix in {".frag", ".vert"} and path not in ignored)
    return standalone + write_combined_image_fragments(temp_dir) + write_combined_warp_vertex_shaders(temp_dir)
...
def main() -> int:
    ...
    with TemporaryDirectory() as directory:
        ...
        programs = [
            ...
            ("transition", GLSL_DIR / "transition.vert", GLSL_DIR / "transition.frag"),
            # The warped surface is the one image program drawn on geometry, so it pairs with its
            # own vertex stage rather than with the shared full-screen quad.
            ("warpSurface", GLSL_DIR / "warpSurface.vert", by_name["warpSurface.frag"]),
        ]
        # The overlay programs take the spliced vertex stage, not the file on disk.
        programs.extend(
            (Path(vertex).stem, by_name[vertex], GLSL_DIR / (Path(vertex).stem + ".frag"))
            for vertex in COMMON_WARP_VERTEX_SHADERS
        )
        programs.extend(
