...
shoot() { # home, png
    ...
    PUNCHSTUDIO_PREFERRED_WIDTH=1600 PUNCHSTUDIO_PREFERRED_HEIGHT=1000 java -Duser.home="$1" --enable-native-access=ALL-UNNAMED -jar PUNCHStudio.jar -state "file://$STATE" > "$OUT/$(basename "$2" .png).log" 2>&1 &
    PID=$!
    sleep "$WAIT"
    python3 - "$PID" "$2" <<'PY'
