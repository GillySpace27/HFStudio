...
record FITSData(Header header, Object pixels, int width, int height, boolean hasBlank, long blank,
                double bzero, double bscale, @Nullable ClipSet.Range headerRange, boolean provisional) {
    ...
    static final float OVER_RANGE_CEILING = 16;
    ...
    ImageBuffer decode(ImageFilter filter, ImageProcessingSettings.FITSParameters state, @Nullable ClipSet.Range clipRange) throws Exception {
        if (pixels instanceof byte[] inData) {
            ...
            return markProvisional(outBuffer.finish());
        }

        ClipSet.Range range = headerRange != null ? headerRange : clipRange;
        if (range == null)
            return markProvisional(ImageBuffer.createWriteBuffer(width, height, ImageBuffer.Format.Gray8, filter).clearPixels().finish());
        ...
        // min/max are already BZERO/BSCALE-corrected physical values (see sampleImage and
        // rawShortToHalfFloat), so no further un-baking is needed once the stretch itself is inverted.
        float span = max - min;
        result.setPhysicalScale(new ImageBuffer.PhysicalScale(min, max,
                inverseMapping(state, span), stretchName(state, span), normalizedMapping(state, span)));
        return markProvisional(result);
    }

    // A frame whose LASCO background could not be fetched decodes and displays, but is flagged so
    // the caller does not cache it as finished.
    private ImageBuffer markProvisional(ImageBuffer buffer) {
        if (provisional)
            buffer.markProvisional();
        return buffer;
    }
    ...
    private short[] rawShortToHalfFloat(float min, double toUnit, DoubleUnaryOperator mapping) {
    ...
    static DoubleUnaryOperator normalizedMapping(ImageProcessingSettings.FITSParameters state, float range) {
    ...
    static DoubleUnaryOperator inverseMapping(ImageProcessingSettings.FITSParameters state, float range) {
        return switch (state.scalingMode()) {
            case Gamma -> {
                double gamma = state.gamma();
    ...
    static String stretchName(ImageProcessingSettings.FITSParameters state, float range) {
        return switch (state.scalingMode()) {
